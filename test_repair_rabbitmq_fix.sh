...
# 修改后的测试脚本，通过调用层面解决 RabbitMQ 防重问题
echo "=== 测试 repair-test 接口（解决 RabbitMQ 防重问题）==="
...
# 函数：生成唯一标识
generate_unique_id() {
    echo "$(date +%s)_$(shuf -i 1000-9999 -n 1)_$(uuidgen | cut -d'-' -f1)"
}
...
send_test_request() {
    local sequence=$1
    local unique_id=$2
    local test_msg="RabbitMQ_Test_${unique_id}"
...
echo ""
echo "=== 第一次测试 ==="
unique_id_1=$(generate_unique_id)
...
if echo "$response1" | jq -e '.success' > /dev/null; then
    ...
    echo ""
    echo "=== 第二次测试 ==="
    unique_id_2=$(generate_unique_id)
