...
type initDeliveryConfig struct {
	clientHome  string
	chainID     string
	validatorID int64
	chain       string
...
// nolint: cyclop
func initDelivery(_ *server.Context, cdc *codec.Codec, initConfig *initDeliveryConfig, config *cfg.Config) error {
	// do not execute init if forceInit is false and genesis.json already exists
	// (or we do not have permission to write to file)
	...
	if chainID == "" {
		chainID = fmt.Sprintf("delivery-%v", common.RandStr(randNameLength))
	}
	...
	WriteDefaultHeimdallConfig(
		filepath.Join(config.RootDir, "config/delivery-config.toml"), helper.GetDefaultHeimdallConfig())
	...
	toPrint := struct {
		ChainID string `json:"chain_id"` // nolint
		NodeID  string `json:"node_id"`  // nolint
	}{
		chainID,
		nodeID,
	}
...
// InitCmd initialises files required to start deilvery.
func initCmd(ctx *server.Context, cdc *codec.Codec) *cobra.Command {
	cmd := &cobra.Command{ // nolint: exhaustivestruct
		...
		RunE: func(_ *cobra.Command, _ []string) error {
			...
			return initDelivery(ctx, cdc, initConfig, config)
		},
	}
