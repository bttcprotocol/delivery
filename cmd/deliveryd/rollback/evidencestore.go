...
func keyLookupFromHeight(height int64) []byte {
...
// It is wrapped by   and PendingEvidence for convenience.
...
func (store *EvidenceStore) listEvidence(prefixKey string, maxNum int64) (evidence []types.Evidence) {
	...
	for ; iter.Valid(); iter.Next() {
		...
		var ei tmev.EvidenceInfo
		err := types.GetCodec().UnmarshalBinaryBare(val, &ei)
		if err != nil {
			panic(err)
		}
...
func (store *EvidenceStore) GetEvidenceInfo(height int64, hash []byte) tmev.EvidenceInfo {
	key := keyLookupFromHeightAndHash(height, hash)
	val := store.db.Get(key)

	if len(val) == 0 {
		return tmev.EvidenceInfo{}
	}
	var ei tmev.EvidenceInfo
...
// Remove all evidences range from org to target height from evidence-lookup
func (store *EvidenceStore) RollbackToVersion(targetHeight int64) {
	heightLimit := targetHeight + 2
	for h := targetHeight + 1; h <= heightLimit; h++ {
		keyHeight := keyLookupFromHeight(h)
		l := store.listEvidence(string(keyHeight), -1)

		for _, evidence := range l {
			key := keyLookup(evidence)
			store.db.Delete(key)
		}
	}
...
func (store *EvidenceStore) getEvidenceInfo(evidence types.Evidence) tmev.EvidenceInfo {
