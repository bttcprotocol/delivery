...
func initCmd(ctx *server.Context, cdc *codec.Codec) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(_ *cobra.Command, _ []string) error {
			...
			return deliveryInit(ctx, cdc, initConfig, config)
		},
	}
...
func deliveryInit(ctx *server.Context, cdc *codec.Codec, initConfig *initDeliveryConfig, config *cfg.Config) error {
	// do not execute init if forceInit is false and mainnet.json already exists (or we do not have permission to write to file)
